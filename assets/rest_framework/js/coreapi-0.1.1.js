(function(f)***REMOVED***if(typeof exports==="object"&&typeof module!=="undefined")***REMOVED***module.exports=f()***REMOVED***else if(typeof define==="function"&&define.amd)***REMOVED***define([],f)***REMOVED***else***REMOVED***var g;if(typeof window!=="undefined")***REMOVED***g=window***REMOVED***else if(typeof global!=="undefined")***REMOVED***g=global***REMOVED***else if(typeof self!=="undefined")***REMOVED***g=self***REMOVED***else***REMOVED***g=this***REMOVED***g.coreapi = f()***REMOVED******REMOVED***)(function()***REMOVED***var define,module,exports;return (function e(t,n,r)***REMOVED***function s(o,u)***REMOVED***if(!n[o])***REMOVED***if(!t[o])***REMOVED***var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f***REMOVED***var l=n[o]=***REMOVED***exports:***REMOVED******REMOVED******REMOVED***;t[o][0].call(l.exports,function(e)***REMOVED***var n=t[o][1][e];return s(n?n:e)***REMOVED***,l,l.exports,e,t,n,r)***REMOVED***return n[o].exports***REMOVED***var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s***REMOVED***)(***REMOVED***1:[function(require,module,exports)***REMOVED***
'use strict';

var _createClass = function () ***REMOVED*** function defineProperties(target, props) ***REMOVED*** for (var i = 0; i < props.length; i++) ***REMOVED*** var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); ***REMOVED*** ***REMOVED*** return function (Constructor, protoProps, staticProps) ***REMOVED*** if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; ***REMOVED***; ***REMOVED***();

function _classCallCheck(instance, Constructor) ***REMOVED*** if (!(instance instanceof Constructor)) ***REMOVED*** throw new TypeError("Cannot call a class as a function"); ***REMOVED*** ***REMOVED***

var BasicAuthentication = function () ***REMOVED***
  function BasicAuthentication() ***REMOVED***
    var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : ***REMOVED******REMOVED***;

    _classCallCheck(this, BasicAuthentication);

    var username = options.username;
    var password = options.password;
    var hash = window.btoa(username + ':' + password);
    this.auth = 'Basic ' + hash;
  ***REMOVED***

  _createClass(BasicAuthentication, [***REMOVED***
    key: 'authenticate',
    value: function authenticate(options) ***REMOVED***
      options.headers['Authorization'] = this.auth;
      return options;
    ***REMOVED***
  ***REMOVED***]);

  return BasicAuthentication;
***REMOVED***();

module.exports = ***REMOVED***
  BasicAuthentication: BasicAuthentication
***REMOVED***;

***REMOVED***,***REMOVED******REMOVED***],2:[function(require,module,exports)***REMOVED***
'use strict';

var basic = require('./basic');
var session = require('./session');
var token = require('./token');

module.exports = ***REMOVED***
  BasicAuthentication: basic.BasicAuthentication,
  SessionAuthentication: session.SessionAuthentication,
  TokenAuthentication: token.TokenAuthentication
***REMOVED***;

***REMOVED***,***REMOVED***"./basic":1,"./session":3,"./token":4***REMOVED***],3:[function(require,module,exports)***REMOVED***
'use strict';

var _createClass = function () ***REMOVED*** function defineProperties(target, props) ***REMOVED*** for (var i = 0; i < props.length; i++) ***REMOVED*** var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); ***REMOVED*** ***REMOVED*** return function (Constructor, protoProps, staticProps) ***REMOVED*** if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; ***REMOVED***; ***REMOVED***();

function _classCallCheck(instance, Constructor) ***REMOVED*** if (!(instance instanceof Constructor)) ***REMOVED*** throw new TypeError("Cannot call a class as a function"); ***REMOVED*** ***REMOVED***

var utils = require('../utils');

function trim(str) ***REMOVED***
  return str.replace(/^\s\s*/, '').replace(/\s\s*$/, '');
***REMOVED***

function getCookie(cookieName, cookieString) ***REMOVED***
  cookieString = cookieString || window.document.cookie;
  if (cookieString && cookieString !== '') ***REMOVED***
    var cookies = cookieString.split(';');
    for (var i = 0; i < cookies.length; i++) ***REMOVED***
      var cookie = trim(cookies[i]);
      // Does this cookie string begin with the name we want?
      if (cookie.substring(0, cookieName.length + 1) === cookieName + '=') ***REMOVED***
        return decodeURIComponent(cookie.substring(cookieName.length + 1));
      ***REMOVED***
    ***REMOVED***
  ***REMOVED***
  return null;
***REMOVED***

var SessionAuthentication = function () ***REMOVED***
  function SessionAuthentication() ***REMOVED***
    var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : ***REMOVED******REMOVED***;

    _classCallCheck(this, SessionAuthentication);

    this.csrfToken = getCookie(options.csrfCookieName, options.cookieString);
    this.csrfHeaderName = options.csrfHeaderName;
  ***REMOVED***

  _createClass(SessionAuthentication, [***REMOVED***
    key: 'authenticate',
    value: function authenticate(options) ***REMOVED***
      options.credentials = 'same-origin';
      if (this.csrfToken && !utils.csrfSafeMethod(options.method)) ***REMOVED***
        options.headers[this.csrfHeaderName] = this.csrfToken;
      ***REMOVED***
      return options;
    ***REMOVED***
  ***REMOVED***]);

  return SessionAuthentication;
***REMOVED***();

module.exports = ***REMOVED***
  SessionAuthentication: SessionAuthentication
***REMOVED***;

***REMOVED***,***REMOVED***"../utils":15***REMOVED***],4:[function(require,module,exports)***REMOVED***
'use strict';

var _createClass = function () ***REMOVED*** function defineProperties(target, props) ***REMOVED*** for (var i = 0; i < props.length; i++) ***REMOVED*** var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); ***REMOVED*** ***REMOVED*** return function (Constructor, protoProps, staticProps) ***REMOVED*** if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; ***REMOVED***; ***REMOVED***();

function _classCallCheck(instance, Constructor) ***REMOVED*** if (!(instance instanceof Constructor)) ***REMOVED*** throw new TypeError("Cannot call a class as a function"); ***REMOVED*** ***REMOVED***

var TokenAuthentication = function () ***REMOVED***
  function TokenAuthentication() ***REMOVED***
    var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : ***REMOVED******REMOVED***;

    _classCallCheck(this, TokenAuthentication);

    this.token = options.token;
    this.scheme = options.scheme || 'Bearer';
  ***REMOVED***

  _createClass(TokenAuthentication, [***REMOVED***
    key: 'authenticate',
    value: function authenticate(options) ***REMOVED***
      options.headers['Authorization'] = this.scheme + ' ' + this.token;
      return options;
    ***REMOVED***
  ***REMOVED***]);

  return TokenAuthentication;
***REMOVED***();

module.exports = ***REMOVED***
  TokenAuthentication: TokenAuthentication
***REMOVED***;

***REMOVED***,***REMOVED******REMOVED***],5:[function(require,module,exports)***REMOVED***
'use strict';

var _createClass = function () ***REMOVED*** function defineProperties(target, props) ***REMOVED*** for (var i = 0; i < props.length; i++) ***REMOVED*** var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); ***REMOVED*** ***REMOVED*** return function (Constructor, protoProps, staticProps) ***REMOVED*** if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; ***REMOVED***; ***REMOVED***();

function _classCallCheck(instance, Constructor) ***REMOVED*** if (!(instance instanceof Constructor)) ***REMOVED*** throw new TypeError("Cannot call a class as a function"); ***REMOVED*** ***REMOVED***

var document = require('./document');
var codecs = require('./codecs');
var errors = require('./errors');
var transports = require('./transports');
var utils = require('./utils');

function lookupLink(node, keys) ***REMOVED***
  var _iteratorNormalCompletion = true;
  var _didIteratorError = false;
  var _iteratorError = undefined;

  try ***REMOVED***
    for (var _iterator = keys[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) ***REMOVED***
      var key = _step.value;

      if (node instanceof document.Document) ***REMOVED***
        node = node.content[key];
      ***REMOVED*** else ***REMOVED***
        node = node[key];
      ***REMOVED***
      if (node === undefined) ***REMOVED***
        throw new errors.LinkLookupError('Invalid link lookup: ' + JSON.stringify(keys));
      ***REMOVED***
    ***REMOVED***
  ***REMOVED*** catch (err) ***REMOVED***
    _didIteratorError = true;
    _iteratorError = err;
  ***REMOVED*** finally ***REMOVED***
    try ***REMOVED***
      if (!_iteratorNormalCompletion && _iterator.return) ***REMOVED***
        _iterator.return();
      ***REMOVED***
    ***REMOVED*** finally ***REMOVED***
      if (_didIteratorError) ***REMOVED***
        throw _iteratorError;
      ***REMOVED***
    ***REMOVED***
  ***REMOVED***

  if (!(node instanceof document.Link)) ***REMOVED***
    throw new errors.LinkLookupError('Invalid link lookup: ' + JSON.stringify(keys));
  ***REMOVED***
  return node;
***REMOVED***

var Client = function () ***REMOVED***
  function Client() ***REMOVED***
    var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : ***REMOVED******REMOVED***;

    _classCallCheck(this, Client);

    var transportOptions = ***REMOVED***
      auth: options.auth || null,
      headers: options.headers || ***REMOVED******REMOVED***,
      requestCallback: options.requestCallback,
      responseCallback: options.responseCallback
    ***REMOVED***;

    this.decoders = options.decoders || [new codecs.CoreJSONCodec(), new codecs.JSONCodec(), new codecs.TextCodec()];
    this.transports = options.transports || [new transports.HTTPTransport(transportOptions)];
  ***REMOVED***

  _createClass(Client, [***REMOVED***
    key: 'action',
    value: function action(document, keys) ***REMOVED***
      var params = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : ***REMOVED******REMOVED***;

      var link = lookupLink(document, keys);
      var transport = utils.determineTransport(this.transports, link.url);
      return transport.action(link, this.decoders, params);
    ***REMOVED***
  ***REMOVED***, ***REMOVED***
    key: 'get',
    value: function get(url) ***REMOVED***
      var link = new document.Link(url, 'get');
      var transport = utils.determineTransport(this.transports, url);
      return transport.action(link, this.decoders);
    ***REMOVED***
  ***REMOVED***]);

  return Client;
***REMOVED***();

module.exports = ***REMOVED***
  Client: Client
***REMOVED***;

***REMOVED***,***REMOVED***"./codecs":7,"./document":10,"./errors":11,"./transports":14,"./utils":15***REMOVED***],6:[function(require,module,exports)***REMOVED***
'use strict';

var _createClass = function () ***REMOVED*** function defineProperties(target, props) ***REMOVED*** for (var i = 0; i < props.length; i++) ***REMOVED*** var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); ***REMOVED*** ***REMOVED*** return function (Constructor, protoProps, staticProps) ***REMOVED*** if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; ***REMOVED***; ***REMOVED***();

var _typeof = typeof Symbol === "function" && typeof Symbol.iterator === "symbol" ? function (obj) ***REMOVED*** return typeof obj; ***REMOVED*** : function (obj) ***REMOVED*** return obj && typeof Symbol === "function" && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; ***REMOVED***;

function _classCallCheck(instance, Constructor) ***REMOVED*** if (!(instance instanceof Constructor)) ***REMOVED*** throw new TypeError("Cannot call a class as a function"); ***REMOVED*** ***REMOVED***

var document = require('../document');
var URL = require('url-parse');

function unescapeKey(key) ***REMOVED***
  if (key.match(/__(type|meta)$/)) ***REMOVED***
    return key.substring(1);
  ***REMOVED***
  return key;
***REMOVED***

function getString(obj, key) ***REMOVED***
  var value = obj[key];
  if (typeof value === 'string') ***REMOVED***
    return value;
  ***REMOVED***
  return '';
***REMOVED***

function getBoolean(obj, key) ***REMOVED***
  var value = obj[key];
  if (typeof value === 'boolean') ***REMOVED***
    return value;
  ***REMOVED***
  return false;
***REMOVED***

function getObject(obj, key) ***REMOVED***
  var value = obj[key];
  if ((typeof value === 'undefined' ? 'undefined' : _typeof(value)) === 'object') ***REMOVED***
    return value;
  ***REMOVED***
  return ***REMOVED******REMOVED***;
***REMOVED***

function getArray(obj, key) ***REMOVED***
  var value = obj[key];
  if (value instanceof Array) ***REMOVED***
    return value;
  ***REMOVED***
  return [];
***REMOVED***

function getContent(data, baseUrl) ***REMOVED***
  var excluded = ['_type', '_meta'];
  var content = ***REMOVED******REMOVED***;
  for (var property in data) ***REMOVED***
    if (data.hasOwnProperty(property) && !excluded.includes(property)) ***REMOVED***
      var key = unescapeKey(property);
      var value = primitiveToNode(data[property], baseUrl);
      content[key] = value;
    ***REMOVED***
  ***REMOVED***
  return content;
***REMOVED***

function primitiveToNode(data, baseUrl) ***REMOVED***
  var isObject = data instanceof Object && !(data instanceof Array);

  if (isObject && data._type === 'document') ***REMOVED***
    // Document
    var meta = getObject(data, '_meta');
    var relativeUrl = getString(meta, 'url');
    var url = relativeUrl ? URL(relativeUrl, baseUrl).toString() : '';
    var title = getString(meta, 'title');
    var description = getString(meta, 'description');
    var content = getContent(data, url);
    return new document.Document(url, title, description, content);
  ***REMOVED*** else if (isObject && data._type === 'link') ***REMOVED***
    // Link
    var _relativeUrl = getString(data, 'url');
    var _url = _relativeUrl ? URL(_relativeUrl, baseUrl).toString() : '';
    var method = getString(data, 'action') || 'get';
    var _title = getString(data, 'title');
    var _description = getString(data, 'description');
    var fieldsData = getArray(data, 'fields');
    var fields = [];
    for (var idx = 0, len = fieldsData.length; idx < len; idx++) ***REMOVED***
      var value = fieldsData[idx];
      var name = getString(value, 'name');
      var required = getBoolean(value, 'required');
      var location = getString(value, 'location');
      var fieldDescription = getString(value, 'fieldDescription');
      var field = new document.Field(name, required, location, fieldDescription);
      fields.push(field);
    ***REMOVED***
    return new document.Link(_url, method, 'application/json', fields, _title, _description);
  ***REMOVED*** else if (isObject) ***REMOVED***
    // Object
    var _content = ***REMOVED******REMOVED***;
    for (var key in data) ***REMOVED***
      if (data.hasOwnProperty(key)) ***REMOVED***
        _content[key] = primitiveToNode(data[key], baseUrl);
      ***REMOVED***
    ***REMOVED***
    return _content;
  ***REMOVED*** else if (data instanceof Array) ***REMOVED***
    // Object
    var _content2 = [];
    for (var _idx = 0, _len = data.length; _idx < _len; _idx++) ***REMOVED***
      _content2.push(primitiveToNode(data[_idx], baseUrl));
    ***REMOVED***
    return _content2;
  ***REMOVED***
  // Primitive
  return data;
***REMOVED***

var CoreJSONCodec = function () ***REMOVED***
  function CoreJSONCodec() ***REMOVED***
    _classCallCheck(this, CoreJSONCodec);

    this.mediaType = 'application/coreapi+json';
  ***REMOVED***

  _createClass(CoreJSONCodec, [***REMOVED***
    key: 'decode',
    value: function decode(text) ***REMOVED***
      var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : ***REMOVED******REMOVED***;

      var data = text;
      if (options.preloaded === undefined || !options.preloaded) ***REMOVED***
        data = JSON.parse(text);
      ***REMOVED***
      return primitiveToNode(data, options.url);
    ***REMOVED***
  ***REMOVED***]);

  return CoreJSONCodec;
***REMOVED***();

module.exports = ***REMOVED***
  CoreJSONCodec: CoreJSONCodec
***REMOVED***;

***REMOVED***,***REMOVED***"../document":10,"url-parse":19***REMOVED***],7:[function(require,module,exports)***REMOVED***
'use strict';

var corejson = require('./corejson');
var json = require('./json');
var text = require('./text');

module.exports = ***REMOVED***
  CoreJSONCodec: corejson.CoreJSONCodec,
  JSONCodec: json.JSONCodec,
  TextCodec: text.TextCodec
***REMOVED***;

***REMOVED***,***REMOVED***"./corejson":6,"./json":8,"./text":9***REMOVED***],8:[function(require,module,exports)***REMOVED***
'use strict';

var _createClass = function () ***REMOVED*** function defineProperties(target, props) ***REMOVED*** for (var i = 0; i < props.length; i++) ***REMOVED*** var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); ***REMOVED*** ***REMOVED*** return function (Constructor, protoProps, staticProps) ***REMOVED*** if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; ***REMOVED***; ***REMOVED***();

function _classCallCheck(instance, Constructor) ***REMOVED*** if (!(instance instanceof Constructor)) ***REMOVED*** throw new TypeError("Cannot call a class as a function"); ***REMOVED*** ***REMOVED***

var JSONCodec = function () ***REMOVED***
  function JSONCodec() ***REMOVED***
    _classCallCheck(this, JSONCodec);

    this.mediaType = 'application/json';
  ***REMOVED***

  _createClass(JSONCodec, [***REMOVED***
    key: 'decode',
    value: function decode(text) ***REMOVED***
      var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : ***REMOVED******REMOVED***;

      return JSON.parse(text);
    ***REMOVED***
  ***REMOVED***]);

  return JSONCodec;
***REMOVED***();

module.exports = ***REMOVED***
  JSONCodec: JSONCodec
***REMOVED***;

***REMOVED***,***REMOVED******REMOVED***],9:[function(require,module,exports)***REMOVED***
'use strict';

var _createClass = function () ***REMOVED*** function defineProperties(target, props) ***REMOVED*** for (var i = 0; i < props.length; i++) ***REMOVED*** var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); ***REMOVED*** ***REMOVED*** return function (Constructor, protoProps, staticProps) ***REMOVED*** if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; ***REMOVED***; ***REMOVED***();

function _classCallCheck(instance, Constructor) ***REMOVED*** if (!(instance instanceof Constructor)) ***REMOVED*** throw new TypeError("Cannot call a class as a function"); ***REMOVED*** ***REMOVED***

var TextCodec = function () ***REMOVED***
  function TextCodec() ***REMOVED***
    _classCallCheck(this, TextCodec);

    this.mediaType = 'text/*';
  ***REMOVED***

  _createClass(TextCodec, [***REMOVED***
    key: 'decode',
    value: function decode(text) ***REMOVED***
      var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : ***REMOVED******REMOVED***;

      return text;
    ***REMOVED***
  ***REMOVED***]);

  return TextCodec;
***REMOVED***();

module.exports = ***REMOVED***
  TextCodec: TextCodec
***REMOVED***;

***REMOVED***,***REMOVED******REMOVED***],10:[function(require,module,exports)***REMOVED***
'use strict';

function _classCallCheck(instance, Constructor) ***REMOVED*** if (!(instance instanceof Constructor)) ***REMOVED*** throw new TypeError("Cannot call a class as a function"); ***REMOVED*** ***REMOVED***

var Document = function Document() ***REMOVED***
  var url = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
  var title = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
  var description = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '';
  var content = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : ***REMOVED******REMOVED***;

  _classCallCheck(this, Document);

  this.url = url;
  this.title = title;
  this.description = description;
  this.content = content;
***REMOVED***;

var Link = function Link(url, method) ***REMOVED***
  var encoding = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'application/json';
  var fields = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : [];
  var title = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : '';
  var description = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : '';

  _classCallCheck(this, Link);

  if (url === undefined) ***REMOVED***
    throw new Error('url argument is required');
  ***REMOVED***

  if (method === undefined) ***REMOVED***
    throw new Error('method argument is required');
  ***REMOVED***

  this.url = url;
  this.method = method;
  this.encoding = encoding;
  this.fields = fields;
  this.title = title;
  this.description = description;
***REMOVED***;

var Field = function Field(name) ***REMOVED***
  var required = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  var location = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '';
  var description = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : '';

  _classCallCheck(this, Field);

  if (name === undefined) ***REMOVED***
    throw new Error('name argument is required');
  ***REMOVED***

  this.name = name;
  this.required = required;
  this.location = location;
  this.description = description;
***REMOVED***;

module.exports = ***REMOVED***
  Document: Document,
  Link: Link,
  Field: Field
***REMOVED***;

***REMOVED***,***REMOVED******REMOVED***],11:[function(require,module,exports)***REMOVED***
'use strict';

function _classCallCheck(instance, Constructor) ***REMOVED*** if (!(instance instanceof Constructor)) ***REMOVED*** throw new TypeError("Cannot call a class as a function"); ***REMOVED*** ***REMOVED***

function _possibleConstructorReturn(self, call) ***REMOVED*** if (!self) ***REMOVED*** throw new ReferenceError("this hasn't been initialised - super() hasn't been called"); ***REMOVED*** return call && (typeof call === "object" || typeof call === "function") ? call : self; ***REMOVED***

function _inherits(subClass, superClass) ***REMOVED*** if (typeof superClass !== "function" && superClass !== null) ***REMOVED*** throw new TypeError("Super expression must either be null or a function, not " + typeof superClass); ***REMOVED*** subClass.prototype = Object.create(superClass && superClass.prototype, ***REMOVED*** constructor: ***REMOVED*** value: subClass, enumerable: false, writable: true, configurable: true ***REMOVED*** ***REMOVED***); if (superClass) Object.setPrototypeOf ? Object.setPrototypeOf(subClass, superClass) : subClass.__proto__ = superClass; ***REMOVED***

var ParameterError = function (_Error) ***REMOVED***
  _inherits(ParameterError, _Error);

  function ParameterError(message) ***REMOVED***
    _classCallCheck(this, ParameterError);

    var _this = _possibleConstructorReturn(this, (ParameterError.__proto__ || Object.getPrototypeOf(ParameterError)).call(this, message));

    _this.message = message;
    _this.name = 'ParameterError';
    return _this;
  ***REMOVED***

  return ParameterError;
***REMOVED***(Error);

var LinkLookupError = function (_Error2) ***REMOVED***
  _inherits(LinkLookupError, _Error2);

  function LinkLookupError(message) ***REMOVED***
    _classCallCheck(this, LinkLookupError);

    var _this2 = _possibleConstructorReturn(this, (LinkLookupError.__proto__ || Object.getPrototypeOf(LinkLookupError)).call(this, message));

    _this2.message = message;
    _this2.name = 'LinkLookupError';
    return _this2;
  ***REMOVED***

  return LinkLookupError;
***REMOVED***(Error);

var ErrorMessage = function (_Error3) ***REMOVED***
  _inherits(ErrorMessage, _Error3);

  function ErrorMessage(message, content) ***REMOVED***
    _classCallCheck(this, ErrorMessage);

    var _this3 = _possibleConstructorReturn(this, (ErrorMessage.__proto__ || Object.getPrototypeOf(ErrorMessage)).call(this, message));

    _this3.message = message;
    _this3.content = content;
    _this3.name = 'ErrorMessage';
    return _this3;
  ***REMOVED***

  return ErrorMessage;
***REMOVED***(Error);

module.exports = ***REMOVED***
  ParameterError: ParameterError,
  LinkLookupError: LinkLookupError,
  ErrorMessage: ErrorMessage
***REMOVED***;

***REMOVED***,***REMOVED******REMOVED***],12:[function(require,module,exports)***REMOVED***
'use strict';

var auth = require('./auth');
var client = require('./client');
var codecs = require('./codecs');
var document = require('./document');
var errors = require('./errors');
var transports = require('./transports');
var utils = require('./utils');

var coreapi = ***REMOVED***
  Client: client.Client,
  Document: document.Document,
  Link: document.Link,
  auth: auth,
  codecs: codecs,
  errors: errors,
  transports: transports,
  utils: utils
***REMOVED***;

module.exports = coreapi;

***REMOVED***,***REMOVED***"./auth":2,"./client":5,"./codecs":7,"./document":10,"./errors":11,"./transports":14,"./utils":15***REMOVED***],13:[function(require,module,exports)***REMOVED***
'use strict';

var _createClass = function () ***REMOVED*** function defineProperties(target, props) ***REMOVED*** for (var i = 0; i < props.length; i++) ***REMOVED*** var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); ***REMOVED*** ***REMOVED*** return function (Constructor, protoProps, staticProps) ***REMOVED*** if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; ***REMOVED***; ***REMOVED***();

function _classCallCheck(instance, Constructor) ***REMOVED*** if (!(instance instanceof Constructor)) ***REMOVED*** throw new TypeError("Cannot call a class as a function"); ***REMOVED*** ***REMOVED***

var fetch = require('isomorphic-fetch');
var errors = require('../errors');
var utils = require('../utils');
var URL = require('url-parse');
var urlTemplate = require('url-template');

var parseResponse = function parseResponse(response, decoders, responseCallback) ***REMOVED***
  return response.text().then(function (text) ***REMOVED***
    if (responseCallback) ***REMOVED***
      responseCallback(response, text);
    ***REMOVED***
    var contentType = response.headers.get('Content-Type');
    var decoder = utils.negotiateDecoder(decoders, contentType);
    var options = ***REMOVED*** url: response.url ***REMOVED***;
    return decoder.decode(text, options);
  ***REMOVED***);
***REMOVED***;

var HTTPTransport = function () ***REMOVED***
  function HTTPTransport() ***REMOVED***
    var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : ***REMOVED******REMOVED***;

    _classCallCheck(this, HTTPTransport);

    this.schemes = ['http', 'https'];
    this.auth = options.auth || null;
    this.headers = options.headers || ***REMOVED******REMOVED***;
    this.fetch = options.fetch || fetch;
    this.FormData = options.FormData || window.FormData;
    this.requestCallback = options.requestCallback;
    this.responseCallback = options.responseCallback;
  ***REMOVED***

  _createClass(HTTPTransport, [***REMOVED***
    key: 'buildRequest',
    value: function buildRequest(link, decoders) ***REMOVED***
      var params = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : ***REMOVED******REMOVED***;

      var fields = link.fields;
      var method = link.method.toUpperCase();
      var queryParams = ***REMOVED******REMOVED***;
      var pathParams = ***REMOVED******REMOVED***;
      var formParams = ***REMOVED******REMOVED***;
      var fieldNames = [];
      var hasBody = false;

      for (var idx = 0, len = fields.length; idx < len; idx++) ***REMOVED***
        var field = fields[idx];

        // Ensure any required fields are included
        if (!params.hasOwnProperty(field.name)) ***REMOVED***
          if (field.required) ***REMOVED***
            throw new errors.ParameterError('Missing required field: "' + field.name + '"');
          ***REMOVED*** else ***REMOVED***
            continue;
          ***REMOVED***
        ***REMOVED***

        fieldNames.push(field.name);
        if (field.location === 'query') ***REMOVED***
          queryParams[field.name] = params[field.name];
        ***REMOVED*** else if (field.location === 'path') ***REMOVED***
          pathParams[field.name] = params[field.name];
        ***REMOVED*** else if (field.location === 'form') ***REMOVED***
          formParams[field.name] = params[field.name];
          hasBody = true;
        ***REMOVED*** else if (field.location === 'body') ***REMOVED***
          formParams = params[field.name];
          hasBody = true;
        ***REMOVED***
      ***REMOVED***

      // Check for any parameters that did not have a matching field
      for (var property in params) ***REMOVED***
        if (params.hasOwnProperty(property) && !fieldNames.includes(property)) ***REMOVED***
          throw new errors.ParameterError('Unknown parameter: "' + property + '"');
        ***REMOVED***
      ***REMOVED***

      var requestOptions = ***REMOVED*** method: method, headers: ***REMOVED******REMOVED*** ***REMOVED***;

      Object.assign(requestOptions.headers, this.headers);

      if (hasBody) ***REMOVED***
        if (link.encoding === 'application/json') ***REMOVED***
          requestOptions.body = JSON.stringify(formParams);
          requestOptions.headers['Content-Type'] = 'application/json';
        ***REMOVED*** else if (link.encoding === 'multipart/form-data') ***REMOVED***
          var form = new this.FormData();

          for (var paramKey in formParams) ***REMOVED***
            form.append(paramKey, formParams[paramKey]);
          ***REMOVED***
          requestOptions.body = form;
        ***REMOVED*** else if (link.encoding === 'application/x-www-form-urlencoded') ***REMOVED***
          var formBody = [];
          for (var _paramKey in formParams) ***REMOVED***
            var encodedKey = encodeURIComponent(_paramKey);
            var encodedValue = encodeURIComponent(formParams[_paramKey]);
            formBody.push(encodedKey + '=' + encodedValue);
          ***REMOVED***
          formBody = formBody.join('&');

          requestOptions.body = formBody;
          requestOptions.headers['Content-Type'] = 'application/x-www-form-urlencoded';
        ***REMOVED***
      ***REMOVED***

      if (this.auth) ***REMOVED***
        requestOptions = this.auth.authenticate(requestOptions);
      ***REMOVED***

      var parsedUrl = urlTemplate.parse(link.url);
      parsedUrl = parsedUrl.expand(pathParams);
      parsedUrl = new URL(parsedUrl);
      parsedUrl.set('query', queryParams);

      return ***REMOVED***
        url: parsedUrl.toString(),
        options: requestOptions
      ***REMOVED***;
    ***REMOVED***
  ***REMOVED***, ***REMOVED***
    key: 'action',
    value: function action(link, decoders) ***REMOVED***
      var params = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : ***REMOVED******REMOVED***;

      var responseCallback = this.responseCallback;
      var request = this.buildRequest(link, decoders, params);

      if (this.requestCallback) ***REMOVED***
        this.requestCallback(request);
      ***REMOVED***

      return this.fetch(request.url, request.options).then(function (response) ***REMOVED***
        if (response.status === 204) ***REMOVED***
          return;
        ***REMOVED***
        return parseResponse(response, decoders, responseCallback).then(function (data) ***REMOVED***
          if (response.ok) ***REMOVED***
            return data;
          ***REMOVED*** else ***REMOVED***
            var title = response.status + ' ' + response.statusText;
            var error = new errors.ErrorMessage(title, data);
            return Promise.reject(error);
          ***REMOVED***
        ***REMOVED***);
      ***REMOVED***);
    ***REMOVED***
  ***REMOVED***]);

  return HTTPTransport;
***REMOVED***();

module.exports = ***REMOVED***
  HTTPTransport: HTTPTransport
***REMOVED***;

***REMOVED***,***REMOVED***"../errors":11,"../utils":15,"isomorphic-fetch":16,"url-parse":19,"url-template":21***REMOVED***],14:[function(require,module,exports)***REMOVED***
'use strict';

var http = require('./http');

module.exports = ***REMOVED***
  HTTPTransport: http.HTTPTransport
***REMOVED***;

***REMOVED***,***REMOVED***"./http":13***REMOVED***],15:[function(require,module,exports)***REMOVED***
'use strict';

var URL = require('url-parse');

var determineTransport = function determineTransport(transports, url) ***REMOVED***
  var parsedUrl = new URL(url);
  var scheme = parsedUrl.protocol.replace(':', '');

  var _iteratorNormalCompletion = true;
  var _didIteratorError = false;
  var _iteratorError = undefined;

  try ***REMOVED***
    for (var _iterator = transports[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) ***REMOVED***
      var transport = _step.value;

      if (transport.schemes.includes(scheme)) ***REMOVED***
        return transport;
      ***REMOVED***
    ***REMOVED***
  ***REMOVED*** catch (err) ***REMOVED***
    _didIteratorError = true;
    _iteratorError = err;
  ***REMOVED*** finally ***REMOVED***
    try ***REMOVED***
      if (!_iteratorNormalCompletion && _iterator.return) ***REMOVED***
        _iterator.return();
      ***REMOVED***
    ***REMOVED*** finally ***REMOVED***
      if (_didIteratorError) ***REMOVED***
        throw _iteratorError;
      ***REMOVED***
    ***REMOVED***
  ***REMOVED***

  throw Error('Unsupported scheme in URL: ' + url);
***REMOVED***;

var negotiateDecoder = function negotiateDecoder(decoders, contentType) ***REMOVED***
  if (contentType === undefined || contentType === null) ***REMOVED***
    return decoders[0];
  ***REMOVED***

  var fullType = contentType.toLowerCase().split(';')[0].trim();
  var mainType = fullType.split('/')[0] + '/*';
  var wildcardType = '*/*';
  var acceptableTypes = [fullType, mainType, wildcardType];

  var _iteratorNormalCompletion2 = true;
  var _didIteratorError2 = false;
  var _iteratorError2 = undefined;

  try ***REMOVED***
    for (var _iterator2 = decoders[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) ***REMOVED***
      var decoder = _step2.value;

      if (acceptableTypes.includes(decoder.mediaType)) ***REMOVED***
        return decoder;
      ***REMOVED***
    ***REMOVED***
  ***REMOVED*** catch (err) ***REMOVED***
    _didIteratorError2 = true;
    _iteratorError2 = err;
  ***REMOVED*** finally ***REMOVED***
    try ***REMOVED***
      if (!_iteratorNormalCompletion2 && _iterator2.return) ***REMOVED***
        _iterator2.return();
      ***REMOVED***
    ***REMOVED*** finally ***REMOVED***
      if (_didIteratorError2) ***REMOVED***
        throw _iteratorError2;
      ***REMOVED***
    ***REMOVED***
  ***REMOVED***

  throw Error('Unsupported media in Content-Type header: ' + contentType);
***REMOVED***;

var csrfSafeMethod = function csrfSafeMethod(method) ***REMOVED***
  // these HTTP methods do not require CSRF protection
  return (/^(GET|HEAD|OPTIONS|TRACE)$/.test(method)
  );
***REMOVED***;

module.exports = ***REMOVED***
  determineTransport: determineTransport,
  negotiateDecoder: negotiateDecoder,
  csrfSafeMethod: csrfSafeMethod
***REMOVED***;

***REMOVED***,***REMOVED***"url-parse":19***REMOVED***],16:[function(require,module,exports)***REMOVED***
// the whatwg-fetch polyfill installs the fetch() function
// on the global object (window or self)
//
// Return that as the export for use in Webpack, Browserify etc.
require('whatwg-fetch');
module.exports = self.fetch.bind(self);

***REMOVED***,***REMOVED***"whatwg-fetch":22***REMOVED***],17:[function(require,module,exports)***REMOVED***
'use strict';

var has = Object.prototype.hasOwnProperty;

/**
 * Simple query string parser.
 *
 * @param ***REMOVED***String***REMOVED*** query The query string that needs to be parsed.
 * @returns ***REMOVED***Object***REMOVED***
 * @api public
 */
function querystring(query) ***REMOVED***
  var parser = /([^=?&]+)=?([^&]*)/g
    , result = ***REMOVED******REMOVED***
    , part;

  //
  // Little nifty parsing hack, leverage the fact that RegExp.exec increments
  // the lastIndex property so we can continue executing this loop until we've
  // parsed all results.
  //
  for (;
    part = parser.exec(query);
    result[decodeURIComponent(part[1])] = decodeURIComponent(part[2])
  );

  return result;
***REMOVED***

/**
 * Transform a query string to an object.
 *
 * @param ***REMOVED***Object***REMOVED*** obj Object that should be transformed.
 * @param ***REMOVED***String***REMOVED*** prefix Optional prefix.
 * @returns ***REMOVED***String***REMOVED***
 * @api public
 */
function querystringify(obj, prefix) ***REMOVED***
  prefix = prefix || '';

  var pairs = [];

  //
  // Optionally prefix with a '?' if needed
  //
  if ('string' !== typeof prefix) prefix = '?';

  for (var key in obj) ***REMOVED***
    if (has.call(obj, key)) ***REMOVED***
      pairs.push(encodeURIComponent(key) +'='+ encodeURIComponent(obj[key]));
    ***REMOVED***
  ***REMOVED***

  return pairs.length ? prefix + pairs.join('&') : '';
***REMOVED***

//
// Expose the module.
//
exports.stringify = querystringify;
exports.parse = querystring;

***REMOVED***,***REMOVED******REMOVED***],18:[function(require,module,exports)***REMOVED***
'use strict';

/**
 * Check if we're required to add a port number.
 *
 * @see https://url.spec.whatwg.org/#default-port
 * @param ***REMOVED***Number|String***REMOVED*** port Port number we need to check
 * @param ***REMOVED***String***REMOVED*** protocol Protocol we need to check against.
 * @returns ***REMOVED***Boolean***REMOVED*** Is it a default port for the given protocol
 * @api private
 */
module.exports = function required(port, protocol) ***REMOVED***
  protocol = protocol.split(':')[0];
  port = +port;

  if (!port) return false;

  switch (protocol) ***REMOVED***
    case 'http':
    case 'ws':
    return port !== 80;

    case 'https':
    case 'wss':
    return port !== 443;

    case 'ftp':
    return port !== 21;

    case 'gopher':
    return port !== 70;

    case 'file':
    return false;
  ***REMOVED***

  return port !== 0;
***REMOVED***;

***REMOVED***,***REMOVED******REMOVED***],19:[function(require,module,exports)***REMOVED***
'use strict';

var required = require('requires-port')
  , lolcation = require('./lolcation')
  , qs = require('querystringify')
  , protocolre = /^([a-z][a-z0-9.+-]*:)?(\/\/)?([\S\s]*)/i;

/**
 * These are the parse rules for the URL parser, it informs the parser
 * about:
 *
 * 0. The char it Needs to parse, if it's a string it should be done using
 *    indexOf, RegExp using exec and NaN means set as current value.
 * 1. The property we should set when parsing this value.
 * 2. Indication if it's backwards or forward parsing, when set as number it's
 *    the value of extra chars that should be split off.
 * 3. Inherit from location if non existing in the parser.
 * 4. `toLowerCase` the resulting value.
 */
var rules = [
  ['#', 'hash'],                        // Extract from the back.
  ['?', 'query'],                       // Extract from the back.
  ['/', 'pathname'],                    // Extract from the back.
  ['@', 'auth', 1],                     // Extract from the front.
  [NaN, 'host', undefined, 1, 1],       // Set left over value.
  [/:(\d+)$/, 'port', undefined, 1],    // RegExp the back.
  [NaN, 'hostname', undefined, 1, 1]    // Set left over.
];

/**
 * @typedef ProtocolExtract
 * @type Object
 * @property ***REMOVED***String***REMOVED*** protocol Protocol matched in the URL, in lowercase.
 * @property ***REMOVED***Boolean***REMOVED*** slashes `true` if protocol is followed by "//", else `false`.
 * @property ***REMOVED***String***REMOVED*** rest Rest of the URL that is not part of the protocol.
 */

/**
 * Extract protocol information from a URL with/without double slash ("//").
 *
 * @param ***REMOVED***String***REMOVED*** address URL we want to extract from.
 * @return ***REMOVED***ProtocolExtract***REMOVED*** Extracted information.
 * @api private
 */
function extractProtocol(address) ***REMOVED***
  var match = protocolre.exec(address);

  return ***REMOVED***
    protocol: match[1] ? match[1].toLowerCase() : '',
    slashes: !!match[2],
    rest: match[3]
  ***REMOVED***;
***REMOVED***

/**
 * Resolve a relative URL pathname against a base URL pathname.
 *
 * @param ***REMOVED***String***REMOVED*** relative Pathname of the relative URL.
 * @param ***REMOVED***String***REMOVED*** base Pathname of the base URL.
 * @return ***REMOVED***String***REMOVED*** Resolved pathname.
 * @api private
 */
function resolve(relative, base) ***REMOVED***
  var path = (base || '/').split('/').slice(0, -1).concat(relative.split('/'))
    , i = path.length
    , last = path[i - 1]
    , unshift = false
    , up = 0;

  while (i--) ***REMOVED***
    if (path[i] === '.') ***REMOVED***
      path.splice(i, 1);
    ***REMOVED*** else if (path[i] === '..') ***REMOVED***
      path.splice(i, 1);
      up++;
    ***REMOVED*** else if (up) ***REMOVED***
      if (i === 0) unshift = true;
      path.splice(i, 1);
      up--;
    ***REMOVED***
  ***REMOVED***

  if (unshift) path.unshift('');
  if (last === '.' || last === '..') path.push('');

  return path.join('/');
***REMOVED***

/**
 * The actual URL instance. Instead of returning an object we've opted-in to
 * create an actual constructor as it's much more memory efficient and
 * faster and it pleases my OCD.
 *
 * @constructor
 * @param ***REMOVED***String***REMOVED*** address URL we want to parse.
 * @param ***REMOVED***Object|String***REMOVED*** location Location defaults for relative paths.
 * @param ***REMOVED***Boolean|Function***REMOVED*** parser Parser for the query string.
 * @api public
 */
function URL(address, location, parser) ***REMOVED***
  if (!(this instanceof URL)) ***REMOVED***
    return new URL(address, location, parser);
  ***REMOVED***

  var relative, extracted, parse, instruction, index, key
    , instructions = rules.slice()
    , type = typeof location
    , url = this
    , i = 0;

  //
  // The following if statements allows this module two have compatibility with
  // 2 different API:
  //
  // 1. Node.js's `url.parse` api which accepts a URL, boolean as arguments
  //    where the boolean indicates that the query string should also be parsed.
  //
  // 2. The `URL` interface of the browser which accepts a URL, object as
  //    arguments. The supplied object will be used as default values / fall-back
  //    for relative paths.
  //
  if ('object' !== type && 'string' !== type) ***REMOVED***
    parser = location;
    location = null;
  ***REMOVED***

  if (parser && 'function' !== typeof parser) parser = qs.parse;

  location = lolcation(location);

  //
  // Extract protocol information before running the instructions.
  //
  extracted = extractProtocol(address || '');
  relative = !extracted.protocol && !extracted.slashes;
  url.slashes = extracted.slashes || relative && location.slashes;
  url.protocol = extracted.protocol || location.protocol || '';
  address = extracted.rest;

  //
  // When the authority component is absent the URL starts with a path
  // component.
  //
  if (!extracted.slashes) instructions[2] = [/(.*)/, 'pathname'];

  for (; i < instructions.length; i++) ***REMOVED***
    instruction = instructions[i];
    parse = instruction[0];
    key = instruction[1];

    if (parse !== parse) ***REMOVED***
      url[key] = address;
    ***REMOVED*** else if ('string' === typeof parse) ***REMOVED***
      if (~(index = address.indexOf(parse))) ***REMOVED***
        if ('number' === typeof instruction[2]) ***REMOVED***
          url[key] = address.slice(0, index);
          address = address.slice(index + instruction[2]);
        ***REMOVED*** else ***REMOVED***
          url[key] = address.slice(index);
          address = address.slice(0, index);
        ***REMOVED***
      ***REMOVED***
    ***REMOVED*** else if (index = parse.exec(address)) ***REMOVED***
      url[key] = index[1];
      address = address.slice(0, index.index);
    ***REMOVED***

    url[key] = url[key] || (
      relative && instruction[3] ? location[key] || '' : ''
    );

    //
    // Hostname, host and protocol should be lowercased so they can be used to
    // create a proper `origin`.
    //
    if (instruction[4]) url[key] = url[key].toLowerCase();
  ***REMOVED***

  //
  // Also parse the supplied query string in to an object. If we're supplied
  // with a custom parser as function use that instead of the default build-in
  // parser.
  //
  if (parser) url.query = parser(url.query);

  //
  // If the URL is relative, resolve the pathname against the base URL.
  //
  if (
      relative
    && location.slashes
    && url.pathname.charAt(0) !== '/'
    && (url.pathname !== '' || location.pathname !== '')
  ) ***REMOVED***
    url.pathname = resolve(url.pathname, location.pathname);
  ***REMOVED***

  //
  // We should not add port numbers if they are already the default port number
  // for a given protocol. As the host also contains the port number we're going
  // override it with the hostname which contains no port number.
  //
  if (!required(url.port, url.protocol)) ***REMOVED***
    url.host = url.hostname;
    url.port = '';
  ***REMOVED***

  //
  // Parse down the `auth` for the username and password.
  //
  url.username = url.password = '';
  if (url.auth) ***REMOVED***
    instruction = url.auth.split(':');
    url.username = instruction[0] || '';
    url.password = instruction[1] || '';
  ***REMOVED***

  url.origin = url.protocol && url.host && url.protocol !== 'file:'
    ? url.protocol +'//'+ url.host
    : 'null';

  //
  // The href is just the compiled result.
  //
  url.href = url.toString();
***REMOVED***

/**
 * This is convenience method for changing properties in the URL instance to
 * insure that they all propagate correctly.
 *
 * @param ***REMOVED***String***REMOVED*** part          Property we need to adjust.
 * @param ***REMOVED***Mixed***REMOVED*** value          The newly assigned value.
 * @param ***REMOVED***Boolean|Function***REMOVED*** fn  When setting the query, it will be the function
 *                               used to parse the query.
 *                               When setting the protocol, double slash will be
 *                               removed from the final url if it is true.
 * @returns ***REMOVED***URL***REMOVED***
 * @api public
 */
URL.prototype.set = function set(part, value, fn) ***REMOVED***
  var url = this;

  switch (part) ***REMOVED***
    case 'query':
      if ('string' === typeof value && value.length) ***REMOVED***
        value = (fn || qs.parse)(value);
      ***REMOVED***

      url[part] = value;
      break;

    case 'port':
      url[part] = value;

      if (!required(value, url.protocol)) ***REMOVED***
        url.host = url.hostname;
        url[part] = '';
      ***REMOVED*** else if (value) ***REMOVED***
        url.host = url.hostname +':'+ value;
      ***REMOVED***

      break;

    case 'hostname':
      url[part] = value;

      if (url.port) value += ':'+ url.port;
      url.host = value;
      break;

    case 'host':
      url[part] = value;

      if (/:\d+$/.test(value)) ***REMOVED***
        value = value.split(':');
        url.port = value.pop();
        url.hostname = value.join(':');
      ***REMOVED*** else ***REMOVED***
        url.hostname = value;
        url.port = '';
      ***REMOVED***

      break;

    case 'protocol':
      url.protocol = value.toLowerCase();
      url.slashes = !fn;
      break;

    case 'pathname':
      url.pathname = value.length && value.charAt(0) !== '/' ? '/' + value : value;

      break;

    default:
      url[part] = value;
  ***REMOVED***

  for (var i = 0; i < rules.length; i++) ***REMOVED***
    var ins = rules[i];

    if (ins[4]) url[ins[1]] = url[ins[1]].toLowerCase();
  ***REMOVED***

  url.origin = url.protocol && url.host && url.protocol !== 'file:'
    ? url.protocol +'//'+ url.host
    : 'null';

  url.href = url.toString();

  return url;
***REMOVED***;

/**
 * Transform the properties back in to a valid and full URL string.
 *
 * @param ***REMOVED***Function***REMOVED*** stringify Optional query stringify function.
 * @returns ***REMOVED***String***REMOVED***
 * @api public
 */
URL.prototype.toString = function toString(stringify) ***REMOVED***
  if (!stringify || 'function' !== typeof stringify) stringify = qs.stringify;

  var query
    , url = this
    , protocol = url.protocol;

  if (protocol && protocol.charAt(protocol.length - 1) !== ':') protocol += ':';

  var result = protocol + (url.slashes ? '//' : '');

  if (url.username) ***REMOVED***
    result += url.username;
    if (url.password) result += ':'+ url.password;
    result += '@';
  ***REMOVED***

  result += url.host + url.pathname;

  query = 'object' === typeof url.query ? stringify(url.query) : url.query;
  if (query) result += '?' !== query.charAt(0) ? '?'+ query : query;

  if (url.hash) result += url.hash;

  return result;
***REMOVED***;

//
// Expose the URL parser and some additional properties that might be useful for
// others or testing.
//
URL.extractProtocol = extractProtocol;
URL.location = lolcation;
URL.qs = qs;

module.exports = URL;

***REMOVED***,***REMOVED***"./lolcation":20,"querystringify":17,"requires-port":18***REMOVED***],20:[function(require,module,exports)***REMOVED***
(function (global)***REMOVED***
'use strict';

var slashes = /^[A-Za-z][A-Za-z0-9+-.]*:\/\//;

/**
 * These properties should not be copied or inherited from. This is only needed
 * for all non blob URL's as a blob URL does not include a hash, only the
 * origin.
 *
 * @type ***REMOVED***Object***REMOVED***
 * @private
 */
var ignore = ***REMOVED*** hash: 1, query: 1 ***REMOVED***
  , URL;

/**
 * The location object differs when your code is loaded through a normal page,
 * Worker or through a worker using a blob. And with the blobble begins the
 * trouble as the location object will contain the URL of the blob, not the
 * location of the page where our code is loaded in. The actual origin is
 * encoded in the `pathname` so we can thankfully generate a good "default"
 * location from it so we can generate proper relative URL's again.
 *
 * @param ***REMOVED***Object|String***REMOVED*** loc Optional default location object.
 * @returns ***REMOVED***Object***REMOVED*** lolcation object.
 * @api public
 */
module.exports = function lolcation(loc) ***REMOVED***
  loc = loc || global.location || ***REMOVED******REMOVED***;
  URL = URL || require('./');

  var finaldestination = ***REMOVED******REMOVED***
    , type = typeof loc
    , key;

  if ('blob:' === loc.protocol) ***REMOVED***
    finaldestination = new URL(unescape(loc.pathname), ***REMOVED******REMOVED***);
  ***REMOVED*** else if ('string' === type) ***REMOVED***
    finaldestination = new URL(loc, ***REMOVED******REMOVED***);
    for (key in ignore) delete finaldestination[key];
  ***REMOVED*** else if ('object' === type) ***REMOVED***
    for (key in loc) ***REMOVED***
      if (key in ignore) continue;
      finaldestination[key] = loc[key];
    ***REMOVED***

    if (finaldestination.slashes === undefined) ***REMOVED***
      finaldestination.slashes = slashes.test(loc.href);
    ***REMOVED***
  ***REMOVED***

  return finaldestination;
***REMOVED***;

***REMOVED***).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : ***REMOVED******REMOVED***)

***REMOVED***,***REMOVED***"./":19***REMOVED***],21:[function(require,module,exports)***REMOVED***
(function (root, factory) ***REMOVED***
    if (typeof exports === 'object') ***REMOVED***
        module.exports = factory();
    ***REMOVED*** else if (typeof define === 'function' && define.amd) ***REMOVED***
        define([], factory);
    ***REMOVED*** else ***REMOVED***
        root.urltemplate = factory();
    ***REMOVED***
***REMOVED***(this, function () ***REMOVED***
  /**
   * @constructor
   */
  function UrlTemplate() ***REMOVED***
  ***REMOVED***

  /**
   * @private
   * @param ***REMOVED***string***REMOVED*** str
   * @return ***REMOVED***string***REMOVED***
   */
  UrlTemplate.prototype.encodeReserved = function (str) ***REMOVED***
    return str.split(/(%[0-9A-Fa-f]***REMOVED***2***REMOVED***)/g).map(function (part) ***REMOVED***
      if (!/%[0-9A-Fa-f]/.test(part)) ***REMOVED***
        part = encodeURI(part).replace(/%5B/g, '[').replace(/%5D/g, ']');
      ***REMOVED***
      return part;
    ***REMOVED***).join('');
  ***REMOVED***;

  /**
   * @private
   * @param ***REMOVED***string***REMOVED*** str
   * @return ***REMOVED***string***REMOVED***
   */
  UrlTemplate.prototype.encodeUnreserved = function (str) ***REMOVED***
    return encodeURIComponent(str).replace(/[!'()*]/g, function (c) ***REMOVED***
      return '%' + c.charCodeAt(0).toString(16).toUpperCase();
    ***REMOVED***);
  ***REMOVED***

  /**
   * @private
   * @param ***REMOVED***string***REMOVED*** operator
   * @param ***REMOVED***string***REMOVED*** value
   * @param ***REMOVED***string***REMOVED*** key
   * @return ***REMOVED***string***REMOVED***
   */
  UrlTemplate.prototype.encodeValue = function (operator, value, key) ***REMOVED***
    value = (operator === '+' || operator === '#') ? this.encodeReserved(value) : this.encodeUnreserved(value);

    if (key) ***REMOVED***
      return this.encodeUnreserved(key) + '=' + value;
    ***REMOVED*** else ***REMOVED***
      return value;
    ***REMOVED***
  ***REMOVED***;

  /**
   * @private
   * @param ***REMOVED*******REMOVED*** value
   * @return ***REMOVED***boolean***REMOVED***
   */
  UrlTemplate.prototype.isDefined = function (value) ***REMOVED***
    return value !== undefined && value !== null;
  ***REMOVED***;

  /**
   * @private
   * @param ***REMOVED***string***REMOVED***
   * @return ***REMOVED***boolean***REMOVED***
   */
  UrlTemplate.prototype.isKeyOperator = function (operator) ***REMOVED***
    return operator === ';' || operator === '&' || operator === '?';
  ***REMOVED***;

  /**
   * @private
   * @param ***REMOVED***Object***REMOVED*** context
   * @param ***REMOVED***string***REMOVED*** operator
   * @param ***REMOVED***string***REMOVED*** key
   * @param ***REMOVED***string***REMOVED*** modifier
   */
  UrlTemplate.prototype.getValues = function (context, operator, key, modifier) ***REMOVED***
    var value = context[key],
        result = [];

    if (this.isDefined(value) && value !== '') ***REMOVED***
      if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean') ***REMOVED***
        value = value.toString();

        if (modifier && modifier !== '*') ***REMOVED***
          value = value.substring(0, parseInt(modifier, 10));
        ***REMOVED***

        result.push(this.encodeValue(operator, value, this.isKeyOperator(operator) ? key : null));
      ***REMOVED*** else ***REMOVED***
        if (modifier === '*') ***REMOVED***
          if (Array.isArray(value)) ***REMOVED***
            value.filter(this.isDefined).forEach(function (value) ***REMOVED***
              result.push(this.encodeValue(operator, value, this.isKeyOperator(operator) ? key : null));
            ***REMOVED***, this);
          ***REMOVED*** else ***REMOVED***
            Object.keys(value).forEach(function (k) ***REMOVED***
              if (this.isDefined(value[k])) ***REMOVED***
                result.push(this.encodeValue(operator, value[k], k));
              ***REMOVED***
            ***REMOVED***, this);
          ***REMOVED***
        ***REMOVED*** else ***REMOVED***
          var tmp = [];

          if (Array.isArray(value)) ***REMOVED***
            value.filter(this.isDefined).forEach(function (value) ***REMOVED***
              tmp.push(this.encodeValue(operator, value));
            ***REMOVED***, this);
          ***REMOVED*** else ***REMOVED***
            Object.keys(value).forEach(function (k) ***REMOVED***
              if (this.isDefined(value[k])) ***REMOVED***
                tmp.push(this.encodeUnreserved(k));
                tmp.push(this.encodeValue(operator, value[k].toString()));
              ***REMOVED***
            ***REMOVED***, this);
          ***REMOVED***

          if (this.isKeyOperator(operator)) ***REMOVED***
            result.push(this.encodeUnreserved(key) + '=' + tmp.join(','));
          ***REMOVED*** else if (tmp.length !== 0) ***REMOVED***
            result.push(tmp.join(','));
          ***REMOVED***
        ***REMOVED***
      ***REMOVED***
    ***REMOVED*** else ***REMOVED***
      if (operator === ';') ***REMOVED***
        if (this.isDefined(value)) ***REMOVED***
          result.push(this.encodeUnreserved(key));
        ***REMOVED***
      ***REMOVED*** else if (value === '' && (operator === '&' || operator === '?')) ***REMOVED***
        result.push(this.encodeUnreserved(key) + '=');
      ***REMOVED*** else if (value === '') ***REMOVED***
        result.push('');
      ***REMOVED***
    ***REMOVED***
    return result;
  ***REMOVED***;

  /**
   * @param ***REMOVED***string***REMOVED*** template
   * @return ***REMOVED***function(Object):string***REMOVED***
   */
  UrlTemplate.prototype.parse = function (template) ***REMOVED***
    var that = this;
    var operators = ['+', '#', '.', '/', ';', '?', '&'];

    return ***REMOVED***
      expand: function (context) ***REMOVED***
        return template.replace(/\***REMOVED***([^\***REMOVED***\***REMOVED***]+)\***REMOVED***|([^\***REMOVED***\***REMOVED***]+)/g, function (_, expression, literal) ***REMOVED***
          if (expression) ***REMOVED***
            var operator = null,
                values = [];

            if (operators.indexOf(expression.charAt(0)) !== -1) ***REMOVED***
              operator = expression.charAt(0);
              expression = expression.substr(1);
            ***REMOVED***

            expression.split(/,/g).forEach(function (variable) ***REMOVED***
              var tmp = /([^:\*]*)(?::(\d+)|(\*))?/.exec(variable);
              values.push.apply(values, that.getValues(context, operator, tmp[1], tmp[2] || tmp[3]));
            ***REMOVED***);

            if (operator && operator !== '+') ***REMOVED***
              var separator = ',';

              if (operator === '?') ***REMOVED***
                separator = '&';
              ***REMOVED*** else if (operator !== '#') ***REMOVED***
                separator = operator;
              ***REMOVED***
              return (values.length !== 0 ? operator : '') + values.join(separator);
            ***REMOVED*** else ***REMOVED***
              return values.join(',');
            ***REMOVED***
          ***REMOVED*** else ***REMOVED***
            return that.encodeReserved(literal);
          ***REMOVED***
        ***REMOVED***);
      ***REMOVED***
    ***REMOVED***;
  ***REMOVED***;

  return new UrlTemplate();
***REMOVED***));

***REMOVED***,***REMOVED******REMOVED***],22:[function(require,module,exports)***REMOVED***
(function(self) ***REMOVED***
  'use strict';

  if (self.fetch) ***REMOVED***
    return
  ***REMOVED***

  var support = ***REMOVED***
    searchParams: 'URLSearchParams' in self,
    iterable: 'Symbol' in self && 'iterator' in Symbol,
    blob: 'FileReader' in self && 'Blob' in self && (function() ***REMOVED***
      try ***REMOVED***
        new Blob()
        return true
      ***REMOVED*** catch(e) ***REMOVED***
        return false
      ***REMOVED***
    ***REMOVED***)(),
    formData: 'FormData' in self,
    arrayBuffer: 'ArrayBuffer' in self
  ***REMOVED***

  if (support.arrayBuffer) ***REMOVED***
    var viewClasses = [
      '[object Int8Array]',
      '[object Uint8Array]',
      '[object Uint8ClampedArray]',
      '[object Int16Array]',
      '[object Uint16Array]',
      '[object Int32Array]',
      '[object Uint32Array]',
      '[object Float32Array]',
      '[object Float64Array]'
    ]

    var isDataView = function(obj) ***REMOVED***
      return obj && DataView.prototype.isPrototypeOf(obj)
    ***REMOVED***

    var isArrayBufferView = ArrayBuffer.isView || function(obj) ***REMOVED***
      return obj && viewClasses.indexOf(Object.prototype.toString.call(obj)) > -1
    ***REMOVED***
  ***REMOVED***

  function normalizeName(name) ***REMOVED***
    if (typeof name !== 'string') ***REMOVED***
      name = String(name)
    ***REMOVED***
    if (/[^a-z0-9\-#$%&'*+.\^_`|~]/i.test(name)) ***REMOVED***
      throw new TypeError('Invalid character in header field name')
    ***REMOVED***
    return name.toLowerCase()
  ***REMOVED***

  function normalizeValue(value) ***REMOVED***
    if (typeof value !== 'string') ***REMOVED***
      value = String(value)
    ***REMOVED***
    return value
  ***REMOVED***

  // Build a destructive iterator for the value list
  function iteratorFor(items) ***REMOVED***
    var iterator = ***REMOVED***
      next: function() ***REMOVED***
        var value = items.shift()
        return ***REMOVED***done: value === undefined, value: value***REMOVED***
      ***REMOVED***
    ***REMOVED***

    if (support.iterable) ***REMOVED***
      iterator[Symbol.iterator] = function() ***REMOVED***
        return iterator
      ***REMOVED***
    ***REMOVED***

    return iterator
  ***REMOVED***

  function Headers(headers) ***REMOVED***
    this.map = ***REMOVED******REMOVED***

    if (headers instanceof Headers) ***REMOVED***
      headers.forEach(function(value, name) ***REMOVED***
        this.append(name, value)
      ***REMOVED***, this)

    ***REMOVED*** else if (headers) ***REMOVED***
      Object.getOwnPropertyNames(headers).forEach(function(name) ***REMOVED***
        this.append(name, headers[name])
      ***REMOVED***, this)
    ***REMOVED***
  ***REMOVED***

  Headers.prototype.append = function(name, value) ***REMOVED***
    name = normalizeName(name)
    value = normalizeValue(value)
    var oldValue = this.map[name]
    this.map[name] = oldValue ? oldValue+','+value : value
  ***REMOVED***

  Headers.prototype['delete'] = function(name) ***REMOVED***
    delete this.map[normalizeName(name)]
  ***REMOVED***

  Headers.prototype.get = function(name) ***REMOVED***
    name = normalizeName(name)
    return this.has(name) ? this.map[name] : null
  ***REMOVED***

  Headers.prototype.has = function(name) ***REMOVED***
    return this.map.hasOwnProperty(normalizeName(name))
  ***REMOVED***

  Headers.prototype.set = function(name, value) ***REMOVED***
    this.map[normalizeName(name)] = normalizeValue(value)
  ***REMOVED***

  Headers.prototype.forEach = function(callback, thisArg) ***REMOVED***
    for (var name in this.map) ***REMOVED***
      if (this.map.hasOwnProperty(name)) ***REMOVED***
        callback.call(thisArg, this.map[name], name, this)
      ***REMOVED***
    ***REMOVED***
  ***REMOVED***

  Headers.prototype.keys = function() ***REMOVED***
    var items = []
    this.forEach(function(value, name) ***REMOVED*** items.push(name) ***REMOVED***)
    return iteratorFor(items)
  ***REMOVED***

  Headers.prototype.values = function() ***REMOVED***
    var items = []
    this.forEach(function(value) ***REMOVED*** items.push(value) ***REMOVED***)
    return iteratorFor(items)
  ***REMOVED***

  Headers.prototype.entries = function() ***REMOVED***
    var items = []
    this.forEach(function(value, name) ***REMOVED*** items.push([name, value]) ***REMOVED***)
    return iteratorFor(items)
  ***REMOVED***

  if (support.iterable) ***REMOVED***
    Headers.prototype[Symbol.iterator] = Headers.prototype.entries
  ***REMOVED***

  function consumed(body) ***REMOVED***
    if (body.bodyUsed) ***REMOVED***
      return Promise.reject(new TypeError('Already read'))
    ***REMOVED***
    body.bodyUsed = true
  ***REMOVED***

  function fileReaderReady(reader) ***REMOVED***
    return new Promise(function(resolve, reject) ***REMOVED***
      reader.onload = function() ***REMOVED***
        resolve(reader.result)
      ***REMOVED***
      reader.onerror = function() ***REMOVED***
        reject(reader.error)
      ***REMOVED***
    ***REMOVED***)
  ***REMOVED***

  function readBlobAsArrayBuffer(blob) ***REMOVED***
    var reader = new FileReader()
    var promise = fileReaderReady(reader)
    reader.readAsArrayBuffer(blob)
    return promise
  ***REMOVED***

  function readBlobAsText(blob) ***REMOVED***
    var reader = new FileReader()
    var promise = fileReaderReady(reader)
    reader.readAsText(blob)
    return promise
  ***REMOVED***

  function bufferClone(buf) ***REMOVED***
    if (buf.slice) ***REMOVED***
      return buf.slice(0)
    ***REMOVED*** else ***REMOVED***
      var view = new Uint8Array(buf.byteLength)
      view.set(new Uint8Array(buf))
      return view.buffer
    ***REMOVED***
  ***REMOVED***

  function Body() ***REMOVED***
    this.bodyUsed = false

    this._initBody = function(body) ***REMOVED***
      this._bodyInit = body
      if (!body) ***REMOVED***
        this._bodyText = ''
      ***REMOVED*** else if (typeof body === 'string') ***REMOVED***
        this._bodyText = body
      ***REMOVED*** else if (support.blob && Blob.prototype.isPrototypeOf(body)) ***REMOVED***
        this._bodyBlob = body
      ***REMOVED*** else if (support.formData && FormData.prototype.isPrototypeOf(body)) ***REMOVED***
        this._bodyFormData = body
      ***REMOVED*** else if (support.searchParams && URLSearchParams.prototype.isPrototypeOf(body)) ***REMOVED***
        this._bodyText = body.toString()
      ***REMOVED*** else if (support.arrayBuffer && support.blob && isDataView(body)) ***REMOVED***
        this._bodyArrayBuffer = bufferClone(body.buffer)
        // IE 10-11 can't handle a DataView body.
        this._bodyInit = new Blob([this._bodyArrayBuffer])
      ***REMOVED*** else if (support.arrayBuffer && (ArrayBuffer.prototype.isPrototypeOf(body) || isArrayBufferView(body))) ***REMOVED***
        this._bodyArrayBuffer = bufferClone(body)
      ***REMOVED*** else ***REMOVED***
        throw new Error('unsupported BodyInit type')
      ***REMOVED***

      if (!this.headers.get('content-type')) ***REMOVED***
        if (typeof body === 'string') ***REMOVED***
          this.headers.set('content-type', 'text/plain;charset=UTF-8')
        ***REMOVED*** else if (this._bodyBlob && this._bodyBlob.type) ***REMOVED***
          this.headers.set('content-type', this._bodyBlob.type)
        ***REMOVED*** else if (support.searchParams && URLSearchParams.prototype.isPrototypeOf(body)) ***REMOVED***
          this.headers.set('content-type', 'application/x-www-form-urlencoded;charset=UTF-8')
        ***REMOVED***
      ***REMOVED***
    ***REMOVED***

    if (support.blob) ***REMOVED***
      this.blob = function() ***REMOVED***
        var rejected = consumed(this)
        if (rejected) ***REMOVED***
          return rejected
        ***REMOVED***

        if (this._bodyBlob) ***REMOVED***
          return Promise.resolve(this._bodyBlob)
        ***REMOVED*** else if (this._bodyArrayBuffer) ***REMOVED***
          return Promise.resolve(new Blob([this._bodyArrayBuffer]))
        ***REMOVED*** else if (this._bodyFormData) ***REMOVED***
          throw new Error('could not read FormData body as blob')
        ***REMOVED*** else ***REMOVED***
          return Promise.resolve(new Blob([this._bodyText]))
        ***REMOVED***
      ***REMOVED***
    ***REMOVED***

    this.text = function() ***REMOVED***
      var rejected = consumed(this)
      if (rejected) ***REMOVED***
        return rejected
      ***REMOVED***

      if (this._bodyBlob) ***REMOVED***
        return readBlobAsText(this._bodyBlob)
      ***REMOVED*** else if (this._bodyArrayBuffer) ***REMOVED***
        var view = new Uint8Array(this._bodyArrayBuffer)
        var str = String.fromCharCode.apply(null, view)
        return Promise.resolve(str)
      ***REMOVED*** else if (this._bodyFormData) ***REMOVED***
        throw new Error('could not read FormData body as text')
      ***REMOVED*** else ***REMOVED***
        return Promise.resolve(this._bodyText)
      ***REMOVED***
    ***REMOVED***

    if (support.arrayBuffer) ***REMOVED***
      this.arrayBuffer = function() ***REMOVED***
        if (this._bodyArrayBuffer) ***REMOVED***
          return consumed(this) || Promise.resolve(this._bodyArrayBuffer)
        ***REMOVED*** else ***REMOVED***
          return this.blob().then(readBlobAsArrayBuffer)
        ***REMOVED***
      ***REMOVED***
    ***REMOVED***

    if (support.formData) ***REMOVED***
      this.formData = function() ***REMOVED***
        return this.text().then(decode)
      ***REMOVED***
    ***REMOVED***

    this.json = function() ***REMOVED***
      return this.text().then(JSON.parse)
    ***REMOVED***

    return this
  ***REMOVED***

  // HTTP methods whose capitalization should be normalized
  var methods = ['DELETE', 'GET', 'HEAD', 'OPTIONS', 'POST', 'PUT']

  function normalizeMethod(method) ***REMOVED***
    var upcased = method.toUpperCase()
    return (methods.indexOf(upcased) > -1) ? upcased : method
  ***REMOVED***

  function Request(input, options) ***REMOVED***
    options = options || ***REMOVED******REMOVED***
    var body = options.body

    if (typeof input === 'string') ***REMOVED***
      this.url = input
    ***REMOVED*** else ***REMOVED***
      if (input.bodyUsed) ***REMOVED***
        throw new TypeError('Already read')
      ***REMOVED***
      this.url = input.url
      this.credentials = input.credentials
      if (!options.headers) ***REMOVED***
        this.headers = new Headers(input.headers)
      ***REMOVED***
      this.method = input.method
      this.mode = input.mode
      if (!body && input._bodyInit != null) ***REMOVED***
        body = input._bodyInit
        input.bodyUsed = true
      ***REMOVED***
    ***REMOVED***

    this.credentials = options.credentials || this.credentials || 'omit'
    if (options.headers || !this.headers) ***REMOVED***
      this.headers = new Headers(options.headers)
    ***REMOVED***
    this.method = normalizeMethod(options.method || this.method || 'GET')
    this.mode = options.mode || this.mode || null
    this.referrer = null

    if ((this.method === 'GET' || this.method === 'HEAD') && body) ***REMOVED***
      throw new TypeError('Body not allowed for GET or HEAD requests')
    ***REMOVED***
    this._initBody(body)
  ***REMOVED***

  Request.prototype.clone = function() ***REMOVED***
    return new Request(this, ***REMOVED*** body: this._bodyInit ***REMOVED***)
  ***REMOVED***

  function decode(body) ***REMOVED***
    var form = new FormData()
    body.trim().split('&').forEach(function(bytes) ***REMOVED***
      if (bytes) ***REMOVED***
        var split = bytes.split('=')
        var name = split.shift().replace(/\+/g, ' ')
        var value = split.join('=').replace(/\+/g, ' ')
        form.append(decodeURIComponent(name), decodeURIComponent(value))
      ***REMOVED***
    ***REMOVED***)
    return form
  ***REMOVED***

  function parseHeaders(rawHeaders) ***REMOVED***
    var headers = new Headers()
    rawHeaders.split('\r\n').forEach(function(line) ***REMOVED***
      var parts = line.split(':')
      var key = parts.shift().trim()
      if (key) ***REMOVED***
        var value = parts.join(':').trim()
        headers.append(key, value)
      ***REMOVED***
    ***REMOVED***)
    return headers
  ***REMOVED***

  Body.call(Request.prototype)

  function Response(bodyInit, options) ***REMOVED***
    if (!options) ***REMOVED***
      options = ***REMOVED******REMOVED***
    ***REMOVED***

    this.type = 'default'
    this.status = 'status' in options ? options.status : 200
    this.ok = this.status >= 200 && this.status < 300
    this.statusText = 'statusText' in options ? options.statusText : 'OK'
    this.headers = new Headers(options.headers)
    this.url = options.url || ''
    this._initBody(bodyInit)
  ***REMOVED***

  Body.call(Response.prototype)

  Response.prototype.clone = function() ***REMOVED***
    return new Response(this._bodyInit, ***REMOVED***
      status: this.status,
      statusText: this.statusText,
      headers: new Headers(this.headers),
      url: this.url
    ***REMOVED***)
  ***REMOVED***

  Response.error = function() ***REMOVED***
    var response = new Response(null, ***REMOVED***status: 0, statusText: ''***REMOVED***)
    response.type = 'error'
    return response
  ***REMOVED***

  var redirectStatuses = [301, 302, 303, 307, 308]

  Response.redirect = function(url, status) ***REMOVED***
    if (redirectStatuses.indexOf(status) === -1) ***REMOVED***
      throw new RangeError('Invalid status code')
    ***REMOVED***

    return new Response(null, ***REMOVED***status: status, headers: ***REMOVED***location: url***REMOVED******REMOVED***)
  ***REMOVED***

  self.Headers = Headers
  self.Request = Request
  self.Response = Response

  self.fetch = function(input, init) ***REMOVED***
    return new Promise(function(resolve, reject) ***REMOVED***
      var request = new Request(input, init)
      var xhr = new XMLHttpRequest()

      xhr.onload = function() ***REMOVED***
        var options = ***REMOVED***
          status: xhr.status,
          statusText: xhr.statusText,
          headers: parseHeaders(xhr.getAllResponseHeaders() || '')
        ***REMOVED***
        options.url = 'responseURL' in xhr ? xhr.responseURL : options.headers.get('X-Request-URL')
        var body = 'response' in xhr ? xhr.response : xhr.responseText
        resolve(new Response(body, options))
      ***REMOVED***

      xhr.onerror = function() ***REMOVED***
        reject(new TypeError('Network request failed'))
      ***REMOVED***

      xhr.ontimeout = function() ***REMOVED***
        reject(new TypeError('Network request failed'))
      ***REMOVED***

      xhr.open(request.method, request.url, true)

      if (request.credentials === 'include') ***REMOVED***
        xhr.withCredentials = true
      ***REMOVED***

      if ('responseType' in xhr && support.blob) ***REMOVED***
        xhr.responseType = 'blob'
      ***REMOVED***

      request.headers.forEach(function(value, name) ***REMOVED***
        xhr.setRequestHeader(name, value)
      ***REMOVED***)

      xhr.send(typeof request._bodyInit === 'undefined' ? null : request._bodyInit)
    ***REMOVED***)
  ***REMOVED***
  self.fetch.polyfill = true
***REMOVED***)(typeof self !== 'undefined' ? self : this);

***REMOVED***,***REMOVED******REMOVED***]***REMOVED***,***REMOVED******REMOVED***,[12])(12)
***REMOVED***);
//# sourceMappingURL=data:application/json;charset=utf-8;base64,